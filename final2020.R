...
#visualize
boxplot(subset_data$y~subset_data$cm2natsm,main='New Mothers Relative TANF vs. Mother in 1-Year National Sample',ylab='relative TANF', xlab='Is mother in 1-year national sample?',names=c("No","Yes"))
boxplot(subset_data$y~subset_data$m2h12,main='New Mothers Relative TANF vs. Supplementary Security Income (SSI) Receival',ylab='relative TANF', xlab='Did mother or child receive SSI?',names=c("No","Yes"))
boxplot(subset_data$y~subset_data$cm2gdad,main='New Mothers Relative TANF vs. Grandfather\'s Presence in Household',ylab='relative TANF', xlab='Is baby\'s grandfather in household?',names=c("No","Yes"))
table(subset_data$m2h9a3)
median_y <- tapply(subset_data$y, subset_data$m2h9a3, median)
subset_data$m2h9a3 <- factor(subset_data$m2h9a3, levels = names(sort(median_y)))
boxplot(subset_data$y ~ subset_data$m2h9a3, 
        main = 'New Mothers Relative TANF vs. Other Social Welfare Receival',
        ylab = 'New Mothers Relative TANF',
        xlab = 'Did you receive other help?',
        names = c('Workers compensation','Unemployment','Other','Medical payment','No','Disability'))

options(scipen=10)
plot(subset_data$y~subset_data$m2l1,main='New Mothers Relative TANF vs. Total Household Income',ylab='relative TANF', xlab='household income ($)')
plot(subset_data$y~subset_data$m2c38,main='New Mothers Relative TANF vs. Father\'s Income',ylab='relative TANF', xlab='father\'s income ($)')
plot(subset_data$y~subset_data$m2b32,main='New Mothers Relative TANF vs. Out-of-Pocket Childcare Expense',ylab='relative TANF', xlab='out-of-pocket childcare expense ($)')
...
plot(test_rf_mod, test_clean$y, main = "Random Forest True vs. Predicted", xlab = "Predicted", ylab = "True",xlim=c(-.04,.06));abline(0,1)
...
ggplot(imp, aes(y=reorder(varnames, mse), weight=mse,fill='pink')) + 
  ...
  ggtitle("Average Percent Increase in Mean-Squared Error (MSE) for each Permuted Feature in Random Forest Regressor")+
